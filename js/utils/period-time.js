...
import { formatDate, formatTemp, formatSpeed,} from './format-data.js';
...
export function handledAppendExtra( extras, position) {
    ...
    const $extraPanel = createDOM(tabExtaWeatherInPanel(extaWeater, position));
    return $extraPanel;
}
...
function tabExtaWeatherInPanel(extras, position) {
    const selected = setHTMLViewSelected(position);
